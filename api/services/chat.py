...
class ChatService:
    ...
    @staticmethod
    def get_chat_bot_config(operator: int, chat_id: int, session: Session) -> AgentConfigModel:
    ...
    @staticmethod
    def get_or_create_bot_chat(operator: int, agent_config_id: int, session: Session) -> ChatModel:
    ...
    @staticmethod
    def create(operator: int, agent_bot_id: int, agent_config_id: int, session: Session) -> ChatModel:
        chat_orm = ChatHelper.create(
            session, operator, agent_bot_id, agent_config_id)
        return ChatModel.model_validate(chat_orm)

    @staticmethod
    def insert_message(message: MessageModelCreate, session: Session) -> MessageModel:
        msg_orm = ChatHelper.insert_message(session, message)
        return MessageModel.model_validate(msg_orm)

    @staticmethod
    def update_message_content(message: MessageModel, content: str, session: Session) -> int:
        flag = ChatHelper.update_message_content(session, message, content)
        return flag

    @staticmethod
    def delete_message(message_id: int, session: Session) -> int:
        flag = ChatHelper.delete_message(session, message_id)
        return flag

    @staticmethod
    def get_message(message_id: int, session: Session) -> MessageModel:
        msg_orm = ChatHelper.get_message(session, message_id)
        return MessageModel.model_validate(msg_orm)

    @staticmethod
    def get_messages(chat_id: int, session: Session) -> List[MessageModel]:
    ...
    @staticmethod
    def clear_messages(chat_id: int, session: Session) -> int:
