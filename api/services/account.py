...
class AccountService:
    ...
    @staticmethod
    async def create(account: AccountCreate, session: Session = Depends(get_db)) -> AccountModel:
        account_orm = await AccountHelper.create(session, account)
        return AccountModel.model_validate(account_orm)

    @staticmethod
    def get_by_id(user_id: int, session: Session = Depends(get_db)) -> AccountModel | None:
    ...
    @staticmethod
    def get_by_email(email: str, session: Session = Depends(get_db)) -> AccountModel | None:
